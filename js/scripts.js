...
let baseHorizontalInc = 100;
let baseVerticalInc = 85;
...
$(document).on('mouseenter','#ks-container #ks-image',function(){
...
$(document).on(touchEvent,'#frontPageGameInstructionsButton',function(){
	// $('#instructionsHelpModal').addClass('is-active');
	var win = window.open('https://drive.google.com/file/d/1fY8-__M2f0QSxvBi0P2oycG6F0l1yJiI/view', '_blank');
	if (win) {
...
$(document).on(touchEvent,'#replaceDuplicateTokens',function(){
...
$(document).on(touchEvent,'#useNatureCube',function(){
...
$(document).on(touchEvent,'#mapContainer #zoomOptions .zoomOption.activeZoom',function(){
	...
	if(!lockFunction) {
		...
		var currentHorizontalPos = parseInt($('#mapContainer #mapHiddenOverlay').css('left'));
		var currentVerticalPos = parseInt($('#mapContainer #mapHiddenOverlay').css('top'));

		
		let baseLeftPos = currentHorizontalPos / (zoomLevel / 10);
		let baseTopPos = currentVerticalPos / (zoomLevel / 10);

		
		let zoomOption = $(this).attr('id');
		...
		let newZoomLeftPos = baseLeftPos * (zoomLevel / 10);
		let newZoomTopPos = baseTopPos * (zoomLevel / 10);


		if(zoomLevel == zoomInLimit) {
			$('#zoomIn').removeClass('activeZoom').addClass('inactiveZoom');
			$('#zoomIn').attr('src', 'img/zoomIn-inactive.png')
		} else if(zoomLevel == zoomOutLimit) {
			$('#zoomOut').attr('src', 'img/zoomOut-inactive.png')
			$('#zoomOut').removeClass('activeZoom').addClass('inactiveZoom');
		} else {

			if(zoomLevel < zoomInLimit) {
				if($('#zoomIn').hasClass('inactiveZoom')) {
					$('#zoomIn').removeClass('inactiveZoom').addClass('activeZoom');
					$('#zoomIn').attr('src', 'img/zoomIn.png')
				}
			}

			if(zoomLevel > zoomOutLimit) {
				if($('#zoomOut').hasClass('inactiveZoom')) {
					$('#zoomOut').removeClass('inactiveZoom').addClass('activeZoom');
					$('#zoomOut').attr('src', 'img/zoomOut.png')
				}
			}

		}

		checkMapConstraints(baseLeftPos, baseTopPos);
...
function setZoom(newZoom,el) {
	...
	transformOrigin = [0,0];
	  ...
	  el.style["transformOrigin"] = '9.4% 5.5%';
	
}

$(document).on(touchEvent,'#mapContainer #mapNavigation .navArrow',function(){
	var targID = $(this).attr('id');
	var direction = targID.split('Arr');
	moveMap(direction[0]);
});
...
$(document).on(touchEvent,'.mapTileContainer.potentialPlacement .tileContainer',function(){
	...
	$('#useNatureCube').attr('disabled', 'disabled');
...
function rotateTileClockwiseFunction() {
	...
	$('.mapTileContainer .tileContainer.lockedIn .tile').css('transform', 'rotate(' + newRotation + 'deg)');
}
...
$(document).on(touchEvent,'.mapTileContainer.placedTile.wildlifeTokenPotential',function(){
	...
	if($('.tokenTileContainer.chosenTokenTileContainer').length) {
		...
		if(keystoneTile) {
			...
			setTimeout(function(){
				// first increase the number of nature cubes by 1
				natureCubesNum++;
				// run the updateNatureCubesNum() function in order to update the number of nature cubes being displayed on the Nature Cube button
				// the false is to signify to NOT deactivate the button, due to the solo tiles still needing to removed AND the tiles and tokens needing to be replenished for the next round
				updateNatureCubesNum(false);
				// the "addCubeAnimation" css class changes the color from green to yellow
				$('#useNatureCube').addClass('addCubeAnimation');
				// the "shake-horizontal" css class shakes the button
				$('#useNatureCube').addClass('shake-horizontal');

				// the 1000 ms time delay is to give the placed token time to animate moving from the display area to the clicked on tile
			}, 1000)
			
			setTimeout(function(){
				// removing the "addCubeAnimation" css class now changes the color from yellow back to the standard default green
				$('#useNatureCube').removeClass('addCubeAnimation');
			}, 1700)

			setTimeout(function(){
				// we remove the "shake-horizontal" since now the shaking animation has concluded
				$('#useNatureCube').removeClass('shake-horizontal');
			}, 2400)
		...
		// NATURE CUBE TILE PLACEMENT CODE
...
$(document).on(touchEvent,'#confirmClearSelectedTokens',function(){
	$('#confirmClearSelectedTokens').attr('disabled', 'disabled');
	$('#selectAllTokensToClear').attr('disabled', 'disabled');

	// now the player will be choosing a token to place, add the .natureCubeToken class to each tile+token container
	$('.tokenTileContainer').addClass('inactive').removeClass('natureCubeClearTokens');
...
function confirmTilePlacement() {
	...
	// check if the newly placed tile should extend the limits of the map
	// if it does, a new column/tile is added (depending on which direction the new tile is added to the existing configuration), and the map is re-rendered, in effect wiping the existing map
	// checkMapConstraints(confirmedTileRow, confirmedTileColumn);
...
function updateNatureCubesNum(activateButton){
	...
	$('.numNatureCubes').html(natureCubesNum);
	...
	if(natureCubesNum == 0) {
		...
		$('#useNatureCube').removeAttr('disabled');
	} else if(!activateButton) {
...
function pickNewTilesTokens() {
	...
	setTimeout(function(){
		...
		// remove the clasws that enforced .75 opacity for the animation stage
...
function removeDuplicateTokens(duplicateAmount) {
	...
	if(duplicateAmount == 4) {
		...
		$('.tokenContainer').each(function(){
			$(this).append('<img class="duplicateToken" src="img/tokens/' + displayedTokens[0] + 'Inactive.png" />');
...
function generateDisplayTile(thisTile) {
	...
	if(thisTile.habitats.length == 1) {
		displayTileHTML+= '<img class="tile" src="img/tiles/' + thisTile.habitats[0] + '.png" />';
	} else if(thisTile.habitats.length == 2) {
		displayTileHTML+= '<img class="tile" src="img/tiles/' + thisTile.habitats[0] + '+' + thisTile.habitats[1] + '.png" style="transform: rotate(' + thisTile.rotation + 'deg);" />';
	}
...
// function addNewColumnRow(direction) {
// 	// in the scenario where adding a new tile to the map requires the map to extend in that direction in order to dynamically stretch so that it never get's filled up by the human player

// 	let previousUp = mapLimits.up;
// 	let previousDown = mapLimits.down;
// 	let previousLeft = mapLimits.left;
// 	let previousRight = mapLimits.right;
...
// 		// example -> new mapLimits.up = 15
// 		mapLimits.up--;

// 		// extending the map up or down requires creating a new row
// 		let newRow = [];

// 		// example -> numColumns = 25 - 14 (11)
// 		let numColumns = mapLimits.right - mapLimits.left;

// 		// loop through the total number of current columns
// 		for (i = 0; i < numColumns; i++) {

// 			// example -> 0 + 14 (14)
// 			let currentColumn = parseInt(i + mapLimits.left);

// 			let thisObj = {
// 				// row: 15 (THIS IS THE NEW ROW WE'RE CREATING)
// 				row: mapLimits.up,
// 				// column: 14 (This column already exists, the code is adding all of the exisiting columns to the new row)
...
// 			// each time a new column is created that's associated with the new row, it's pushed into the "newRow" array
// 			newRow.push(thisObj);

// 			// MAP GENERATION CHANGE!
// 			$('#row-' + previousUp + '-column-' + previousLeft).before('<div id="row-' + mapLimits.up + '-column-' + currentColumn +  '" class="mapTileContainer row-' + mapLimits.up + ' column-' + currentColumn +  '"><img class="tileOutline" src="img/tileOutline.png"></div>');
			
// 		}

// 		// once all of the existing columns have been created and assigned to the the new row, the new row is inserted AT THE START of the "mapData" (because of the "up" direction, this row will be inserted BEFORE all the others)
// 		mapData.unshift(newRow);
...
// 			let thisObj = {
// 				// row: 27 (THIS IS THE NEW ROW WE'RE CREATING)
// 				row: mapLimits.down,
// 				// column: 14 (This column already exists, the code is adding all of the exisiting columns to the new row)
// 				column: currentColumn,
// 				placedTile: false,
...
// 			// MAP GENERATION CHANGE!
// 			$('#mapContainer #mapHiddenOverlay').append('<div id="row-' + mapLimits.down + '-column-' + currentColumn +  '" class="mapTileContainer row-' + mapLimits.down + ' column-' + currentColumn +  '"><img class="tileOutline" src="img/tileOutline.png"></div>');

// 		}

// 		// once all of the existing columns have been created and assigned to the the new row, the new row is inserted AT THE END of the "mapData" (because of the "down" direction, this row will be inserted AFTER all the others)
// 		mapData.push(newRow);
...
// 		// example -> new mapLimits.left = 13
// 		mapLimits.left--;

// 		// looping though the top level of mapData only loops through the rows
// 		for (i = 0; i < mapData.length; i++) {

// 			// extending the map left or right requires creating a new column'
// 			// 0 + 16 = 16
// 			let currentRow = parseInt(i + mapLimits.up);

// 			// by looping through the top level of the "mapData" array, each row is going to have the new column added to it
// 			let thisObj = {
// 				// row: 16,
// 				row: currentRow,
// 				// column: 13,
// 				column: mapLimits.left,
// 				placedTile: false,
// 				habitats: [],
// 				wildlife: [],
// 				placedToken: false,
...
// 			// this code is INSIDE the for loop, because before the code moves on to the next row, it needs to first add the new column
// 			// the new column is inserted AT THE START of the current "mapData" row (because of the "left" direction, this column will be inserted BEFORE all the others)
// 			mapData[i].unshift(thisObj);

// 			// MAP GENERATION CHANGE!
// 			$('#row-' + currentRow + '-column-' + previousLeft).before('<div id="row-' + currentRow + '-column-' + mapLimits.left +  '" class="mapTileContainer row-' + currentRow + ' column-' + mapLimits.left +  '"><img class="tileOutline" src="img/tileOutline.png"></div>');
...
// 		// example -> new mapLimits.right = 25
// 		mapLimits.right++;
...
// 			// this code is INSIDE the for loop, because before the code moves on to the next row, it needs to first add the new column
// 			// the new column is inserted AT THE END of the current "mapData" row (because of the "right" direction, this column will be inserted AFTER all the others)
// 			mapData[i].push(thisObj);

// 			// MAP GENERATION CHANGE!
// 			$('#row-' + currentRow + '-column-' + previousRight).after('<div id="row-' + currentRow + '-column-' + mapLimits.right +  '" class="mapTileContainer row-' + currentRow + ' column-' + mapLimits.right +  '"><img class="tileOutline" src="img/tileOutline.png"></div>');
...
// 	// after adding the new row / column, the code re-generates the map in order to add the new row / columns into the DOM
// 	// generateMap();
...
function generateMap() {
	...
	for (let i = 0; i < mapData.length; i++) {
		for (let j = 0; j < mapData[i].length; j++) {
			...
			if(mapData[i][j].placedTile) {
				...
				if(mapData[i][j].habitats.length == 1) {
					mapHTML +=  '<img class="tile" src="img/tiles/' + mapData[i][j].habitats + '.png" style="transform: rotate(' + mapData[i][j].rotation + 'deg);">';
				} else {
					mapHTML +=  '<img class="tile" src="img/tiles/' + mapData[i][j].habitats[0] + '+' + mapData[i][j].habitats[1] + '.png" style="transform: rotate(' + mapData[i][j].rotation + 'deg);">';
				}
	...
	mapHTML += '<div id="zoomOptions">';

	mapHTML += '<img id="zoomIn" class="zoomOption inactiveZoom" src="img/zoomIn-inactive.png" />';
	mapHTML += '<img id="zoomOut" class="zoomOption activeZoom" src="img/zoomOut.png" />';

	mapHTML += '</div>';

	mapHTML += '<div id="mapNavigation">';

	mapHTML += '<img id="navBackground" src="img/woodCircle.png" />';

	mapHTML += '<img id="upArrow" class="navArrow" src="img/arrow.png" />';
	mapHTML += '<img id="rightArrow" class="navArrow" src="img/arrow.png" />';
	mapHTML += '<img id="downArrow" class="navArrow" src="img/arrow.png" />';
	mapHTML += '<img id="leftArrow" class="navArrow" src="img/arrow.png" />';
...
function moveMap(direction) {
	...
	// add the "activeArrow" class to the arrow that matches the direction to give it a small animation
	$('#' + direction + 'Arrow').addClass('activeArrow');

	// remove the "activeArrow" class to figgle it back to it's original starting position
	setTimeout(function(){
		$('#' + direction + 'Arrow').removeClass('activeArrow');
	}, 100)

	// baseHorizontalInc = 100
	let horizontalInc = baseHorizontalInc * (zoomLevel / 10);

	// baseVerticalInc = 115
	let verticalInc = baseVerticalInc * (zoomLevel / 10);
	
	// horizontalInc = "100"
	// verticalInc = "115"


	// the increments for the map to be move in any direction is 100px (this is the square dimensions of each hex)
	
	// store the current vertical and horizontal positions in variables
	// horizontal = 1643.2;
	// vertical = 1581;

	var oldVerticalPos = parseInt($('#mapContainer #mapHiddenOverlay').css('top'));
	var oldHorizontalPos = parseInt($('#mapContainer #mapHiddenOverlay').css('left'));
	
	// create the "newVertical" + "newHorizontal" variables to store the new values
	var newVerticalPos = oldVerticalPos;
	var newHorizontalPos = oldHorizontalPos;

	if(direction == 'up' || direction == 'down') {	
		if(direction == 'up') {
			// newVerticalPos = 1581 - 100 (1481);
			newVerticalPos = oldVerticalPos + verticalInc;
		} else if(direction == 'down') {	
			// newVerticalPos = 1581 + 100 (1681);
			newVerticalPos = oldVerticalPos - verticalInc;		
		}

		let baseTopPos = newVerticalPos / (zoomLevel / 10);
		let topIncs = baseTopPos / baseVerticalInc;
		let newTopPos = checkMapLimits('vertical', topIncs, baseTopPos);
	...
	if(direction == 'left' || direction == 'right') {	
		if(direction == 'left') {
			// newHorizontalPos = 1643.2 - 100; (1543.2)
			newHorizontalPos = oldHorizontalPos + horizontalInc;
		} else if(direction == 'right') {
			// newHorizontalPos = 1643.2 + 100 (1743.2);
			newHorizontalPos = oldHorizontalPos - horizontalInc;		
		}

		let baseLeftPos = newHorizontalPos / (zoomLevel / 10);
		let leftIncs = baseLeftPos / baseHorizontalInc;
		let newLeftPos = checkMapLimits('horizontal', leftIncs, baseLeftPos);
...
function checkMapConstraints(thisLeft, thisTop) {
	let leftIncs = thisLeft / (baseHorizontalInc * (zoomLevel / 10)); // -6 or 0 or 6
	...
	let topIncs = thisTop / (baseVerticalInc * (zoomLevel / 10)) // -6 or 0 or 6


	let newLeftPos = checkMapLimits('horizontal', leftIncs, thisLeft);


	let newTopPos = checkMapLimits('vertical', topIncs, thisTop);


	// apply the new positions to the map
	$('#mapContainer #mapHiddenOverlay').css('left', newLeftPos);
	$('#mapContainer #mapHiddenOverlay').css('top', newTopPos);
}

function checkMapLimits(direction, currentIncs, originalPos){
	...
	if(direction == 'horizontal') {
		...
		if(currentIncs <= (mapStats.centerColumn - mapLimits.right) + (horizontalTilesHalfwayAmount) + 1) {

			mapStats.directionStatus.right = 'mapLimit-locked';
			$('#rightArrow').hide();

			return originalPos * (zoomLevel / 10);
			
		} else if(currentIncs <= (mapStats.tileExtremes.column.right - horizontalTilesHalfwayAmount)) {
		...
		} else if(currentIncs > (mapStats.centerColumn - mapLimits.left) - (horizontalTilesHalfwayAmount + 1)) {
		...
		} else if((currentIncs + 1) >= (mapStats.tileExtremes.column.left + horizontalTilesHalfwayAmount)) {
		...
		// verticalTileLimit = "6"
		// verticalTilesHalfwayAmount = "3"
		// currentIncs = "0"
		// (mapStats.tileExtremes.row.top = "0"
		// (mapStats.tileExtremes.row.bottom = "1"
	
		// (mapLimits.up = "9"
		// (mapLimits.down = "31"
		// (mapStats.centerRow = "20"
		...
		if(currentIncs <= (mapStats.centerRow - mapLimits.down) + (verticalTilesHalfwayAmount) + 1) {
		...
		} else if((currentIncs - 1) <= (mapStats.tileExtremes.row.bottom - verticalTilesHalfwayAmount)) {
		...
		} else if(currentIncs > (mapStats.centerRow - mapLimits.up) - (verticalTilesHalfwayAmount + 1)) {
		...
		} else if(currentIncs >= (mapStats.tileExtremes.row.top + verticalTilesHalfwayAmount)) {
...
jQuery.fn.extend({
    ...
    parentToAnimate: function(newParent, duration) {
		...
		if($element[0].className == 'tileContainer' || $element[0].className == 'tileContainer lockedIn' || $element[0].className == 'tileContainer chosenNatureCubeTile lockedIn' || $element[0].className == 'activeToken') {
...
function endOfGameSetup() {
	...
	setTimeout(function(){
		...
		$('#gameButtonsContainer').remove();
		$('#turnsLeftContainer').remove();
		...
		<div id="scoringTable-finalScoringContainer" class="finalScoringItem inactiveScoringItem">
					...
					<tbody>
						...
						<tr>		
							<td class="scoreCell">
								...
								<!--input autocomplete="off" id="bear-wildlifeScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="bearFinalScore" category="wildlife-Cell"-->
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<p class="scoreCellText scoringInput dataFromText" id="elk-wildlifeScoringInput" category="wildlife-Cell"></p>	
								<!--input autocomplete="off" id="elk-wildlifeScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="elkFinalScore" category="wildlife-Cell"-->
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<p class="scoreCellText scoringInput dataFromText" id="fox-wildlifeScoringInput" category="wildlife-Cell"></p>	
								<!--input autocomplete="off" id="fox-wildlifeScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="foxFinalScore" category="wildlife-Cell"-->
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<p class="scoreCellText scoringInput dataFromText" id="hawk-wildlifeScoringInput" category="wildlife-Cell"></p>	
								<!--input autocomplete="off" id="hawk-wildlifeScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="hawkFinalScore" category="wildlife-Cell"-->
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<p class="scoreCellText scoringInput dataFromText" id="salmon-wildlifeScoringInput" category="wildlife-Cell"></p>	
								<!--input autocomplete="off" id="salmon-wildlifeScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="salmonFinalScore" category="wildlife-Cell"-->
							</td>
						</tr>
						<tr>		
				...
				<table id="habitatScoringTable" cellpadding="0" cellspacing="0" class="finalScoringTable">
					<tbody>
						<tr>		
							<td class="scoreCell">
								<!--input autocomplete="off" id="mountain-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="mountainFinalScore" category="habitat-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="mountain-habitatScoringInput" category="habitat-Cell"></p>
							</td>
							<td class="scoreCell">
								<!--input autocomplete="off" id="mountainBonus-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="mountainBonusFinalScore" category="habitatBonus-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="mountainBonus-habitatScoringInput" category="habitatBonus-Cell"></p>
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<!--input autocomplete="off" id="forest-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="forestFinalScore" category="habitat-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="forest-habitatScoringInput" category="habitat-Cell"></p>
							</td>
							<td class="scoreCell">
								<!--input autocomplete="off" id="forestBonus-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="forestBonusFinalScore" category="habitatBonus-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="forestBonus-habitatScoringInput" category="habitatBonus-Cell"></p>
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<!--input autocomplete="off" id="desert-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="desertFinalScore" category="habitat-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="desert-habitatScoringInput" category="habitat-Cell"></p>
							</td>
							<td class="scoreCell">
								<!--input autocomplete="off" id="desertBonus-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="desertBonusFinalScore" category="habitatBonus-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="desertBonus-habitatScoringInput" category="habitatBonus-Cell"></p>
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<!--input autocomplete="off" id="swamp-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="swampFinalScore" category="habitat-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="swamp-habitatScoringInput" category="habitat-Cell"></p>
							</td>
							<td class="scoreCell">
								<!--input autocomplete="off" id="swampBonus-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="swampBonusFinalScore" category="habitatBonus-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="swampBonus-habitatScoringInput" category="habitatBonus-Cell"></p>
							</td>
						</tr>
						<tr>		
							<td class="scoreCell">
								<!--input autocomplete="off" id="lake-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="lakeFinalScore" category="habitat-Cell"-->
								<p class="scoreCellText scoringInput dataFromText" id="lake-habitatScoringInput" category="habitat-Cell"></p>
							</td>
							<td class="scoreCell">
								<!--input autocomplete="off" id="lakeBonus-habitatScoringInput" class="scoringInput dataFromInput" inputmode="numeric" pattern="[0-9]*" type="text" name="lakeBonusFinalScore" category="habitatBonus-Cell"-->
	...
	setTimeout(function(){
		...
		$('#scoringTable-FinalScoringModalTrigger').prepend('<img class="activeScoringFrame" src="img/scoringGoal-active-alt2.png" />');
...
$(document).on('mouseover','.finalScoringItemTrigger:not(.lockTrigger).activeScoringItemTrigger .goalScoringTransparentLayer',function(){
	...
	$(this).after('<img class="hoverScoringFrame" src="img/scoringGoal-hover-alt2.png" />');
...
$(document).on(touchEvent,'.finalScoringItemTrigger:not(.lockTrigger).activeScoringItemTrigger .goalScoringTransparentLayer',function(){
	...
	setTimeout(function(){
		...
		$('#' + parentID).prepend('<img class="activeScoringFrame" src="img/scoringGoal-active-alt2.png" />')
		$('.activeScoringFrame').fadeIn('fast');
	}, 200)
...
// $(document).on('keyup','.finalScoringTable .scoreCell .scoringInput',function(){
...
// 			let cellCategory = $(this).attr('category');
// 			let thisTable = $(this).closest('.finalScoringTable').attr('id');
// 			updateSubtotal(thisTable, cellCategory);
// 			if(cellCategory == 'wildlife-Cell' && cellID != 'natureTokensScoringInput') {
// 				let wildlifeToUpdate = cellID.split('-');
// 				updateLinkedWildlifeStats(wildlifeToUpdate[0], 'mainTable')
// 			}
// 		}
// 	}
...
// $(document).on('keyup','.largeWildlifeFinalScoringInfo.finalScoringItem .individualWildlifeScoringInputContainer .individualScoringInput',function(){
...
// 			let cellID = $(this).attr('id');
// 			let wildlifeToUpdate = cellID.split('-');
// 			updateLinkedWildlifeStats(wildlifeToUpdate[0], 'individualScoring');
// 			updateSubtotal('wildlifeScoringTable', 'wildlife-Cell');
// 		}
// 	}
...
// function updateLinkedWildlifeStats(thisWildlife, updateOrigin) {
// 	if(updateOrigin == 'mainTable') {
// 		let newVal = $('#' + thisWildlife + '-wildlifeScoringInput').val()
// 		$('#' + thisWildlife + '-individualWildlifeScoringInput').val(newVal);
// 	} else if(updateOrigin == 'individualScoring') {
// 		let newVal = $('#' + thisWildlife + '-individualWildlifeScoringInput').val()
// 		$('#' + thisWildlife + '-wildlifeScoringInput').val(newVal);
// 	}
// }
...
function calculateElkTokenScoring() {
	...
	let thisLabel = 'calculateElkTokenScoring function';

	console.group(thisLabel);
	...
	let thisLabelTwo = 'const tokenID of tokenIDs Loop';

	console.group(thisLabelTwo);
			...
			if(usedTokenIDs.indexOf(tokenID) === -1) {
				...
				if (neighbourElkTokens && neighbourElkTokens.length) {
					if(neighbourElkTokens.length == 1) {
						...
						let thisLabelFive = 'Generate all Elk Lines to check'

						console.group(thisLabelFive);
						...
						for (let i = 0; i < neighbourElkTokens.length; i++) {
							...
							let tempConnectedElkTilesInLine = allTokensInLine(neighbourElkTokens[i], tempDirection, 'elk');
						...
						console.groupEnd(thisLabelFive);
	...
	console.groupEnd(thisLabelTwo);

	let thisLabelThree = 'Process all stored Elk lines';

	console.group(thisLabelThree);
	...
	if(allPotentialElkLines.length > 0) {
		...
		for (let i = 0; i < allPotentialElkLines.length; i++) {
			...
			if(elkInLineNum > 4) elkInLineNum = 4;
...
let validSalmonRun = false;
	
function calculateSalmonTokenScoring() {

	let calculateSalmonTokenScoringLabel = 'calculateSalmonTokenScoring';
	console.group(calculateSalmonTokenScoringLabel);
	...
	for (const tokenID of tokenIDs) {
		...
		if(allPlacedTokens[tokenID] == 'salmon') {
			...
			if(usedSalmonTokenIDs.indexOf(tokenID) == -1) {

				let neighbourTiles = neighbourTileIDs(tokenID);
				let neighbourSalmonTiles = [];
				
				for (let i = 0; i < neighbourTiles.length; i++) {
					if(allPlacedTokens.hasOwnProperty(neighbourTiles[i])) {
						if(allPlacedTokens[neighbourTiles[i]] == 'salmon') {
							// Should check straight off the bat how many Salmon tiles neighbour the initial tile
							if(usedSalmonTokenIDs.indexOf(neighbourTiles[i]) == -1) {
								neighbourSalmonTiles.push(neighbourTiles[i]);
							} else {
							}
						}
					}
				...
				if(neighbourSalmonTiles.length == 2) {
					let tilesToCheck = [tokenID];
					tilesToCheck.push(...neighbourSalmonTiles);

					let firstNeighbourTiles = neighbourTileIDs(neighbourSalmonTiles[0]);
					let secondNeighbourTiles = neighbourTileIDs(neighbourSalmonTiles[1]);

					

					if(firstNeighbourTiles.indexOf(neighbourSalmonTiles[1]) === -1 && secondNeighbourTiles.indexOf(neighbourSalmonTiles[0]) === -1) {
						// perform a run forwards and backwards!!
						let forwardsAndBackwardsSalmonRunIDs = forwardsAndBackwardsSalmonRun(tokenID, neighbourSalmonTiles);
						if(validSalmonRun) {
							potentialSalmonTokenIDs.push(...forwardsAndBackwardsSalmonRunIDs);
						}
							
					} else {
						checkForTriangleSalmonRun(tilesToCheck);
						if(validSalmonRun) {
							potentialSalmonTokenIDs.push(...tilesToCheck)
						}
					}
					
				} else if(neighbourSalmonTiles.length < 2) {
					potentialSalmonTokenIDs.push(tokenID);
					if(neighbourSalmonTiles.length == 1) {
						searchSalmonNeighbours: // The first for loop is labeled "searchSalmonNeighbours"
						for (let j = 0; j < neighbourSalmonTiles.length; j++) {
							let salmonRunIDs = salmonTokensInRun(tokenID, 'salmon');
							if(!validSalmonRun) break searchSalmonNeighbours;
							for (let i = 0; i < salmonRunIDs.length; i++) {
							}
				...
				if(validSalmonRun) confirmedSalmonRuns.push(potentialSalmonTokenIDs);
...
function forwardsAndBackwardsSalmonRun(firstTile, startingTiles){
	...
	let salmonTokensInRunLabel = 'salmonTokensInRun function';
	console.group(salmonTokensInRunLabel);
	...
	for (let i = 0; i < startingTiles.length; i++) {
		...
		while (!forwardsBackwardsSalmonRunEnded) {
			...
			result = searchNeighbourTilesForWildlife(nextTokenID, 'salmon');
			...
			if(result.length > 0 && result.length < 3) {
				...
				if(result.length == 1) {
					...
					if(result.length != 0) {
						usedSalmonTokenIDs.push(...result);
					}
				...
				usedSalmonTokenIDs.push(...result);
				validSalmonRun = false;
				forwardsBackwardsSalmonRunEnded = true;
			}
		}
	}

	console.groupEnd(salmonTokensInRunLabel);
...
function salmonTokensInRun(startID, thisWildlife) {
	...
	while (!salmonRunEnded) {
		...
		result = searchNeighbourTilesForWildlife(nextTokenID, thisWildlife);
